...
import { PlayCircle, Eye, MessageSquare, Plus, Loader2, ArrowRight, Clock, CheckCircle, Mic, Filter } from "lucide-react";
        ...
        <div className="space-y-6 pb-10">
            ...
            {sessions.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                        ...
                        className={`text-xs font-medium px-3 py-1.5 rounded-full transition-colors ${activeFilter === 'all' ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300'}`}
                    >
                        All ({sessions.length})
                    </button>
                    {Array.from(new Set(sessions.map(s => s.interview_type || 'general'))).map(type => {
                        const count = sessions.filter(s => (s.interview_type || 'general') === type).length;
                        return (
                            <button
                                key={type}
                                onClick={() => setActiveFilter(type)}
                                className={`text-xs font-medium px-3 py-1.5 rounded-full transition-colors capitalize ${activeFilter === type ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300'}`}
                            >
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    {sessions
                        .filter(s => activeFilter === 'all' || (s.interview_type || 'general') === activeFilter)
                        .map((session) => {
